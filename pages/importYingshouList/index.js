...
Page({
  ...
  onAllSelect(e) {
    ...
    if (!!checked) {
      filterList = this.data.filterList.map(item => ({ ...item,
        checked: true
      }));
    } else {
      filterList = this.data.filterList.map(item => ({ ...item,
  ...
  searchResultUseTime(startTime, endTime) {
    ...
    const filterList = this.data.tempImportList.filter(item => {
      ...
      if (!startTime && !!endTime) {
        ...
        return startTime <= new Date(businessDateTime).getTime() && new Date(businessDateTime).getTime() <= endTime;
      }
    });
    this.setData({
      filterList,
  ...
  onCheckboxSubmit() {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (!!arr[i].receivablebillCode) // 应收单单号
        temp.receivablebillCode = arr[i].receivablebillCode;else // 应付单单号
