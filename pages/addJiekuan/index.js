...
Page({
  ...
  getCapitalDetailIdFromStorage() {
    // 从缓存里获取科目id
    const capital = tt.getStorageSync('capital');
  ...
  onShow() {
    ...
    this.getCapitalDetailIdFromStorage(); // 页面显示
  ...
  handleAddBorrow() {
    console.log(this.data.submitData);
    const borrowAmountIndex = tt.getStorageSync('borrowAmountIndex');
    const numberReg = /^\d+(\.\d+)?$/;
  ...
  getIncomeBankList(applicantType, applicantId, incomeBankName) {
    ...
    request({
      ...
      method: 'GET',
      success: res => {
        var arr = res.data.obj; // edit的时候，设置incomeBankIndex
  ...
  setSelectedAuxpty(auxpty) {
    this.setData({
      selectedAuxpty: { ...this.data.selectedAuxpty,
        [auxpty.auxptyId]: {
          id: auxpty.id,
          name: auxpty.name,
  ...
  setRenderData(data) {
    // 请求
    this.getAccountbookList(data); // billDetailList
    ...
    this.getSubjectAuxptyList(data.subjectId, data.accountbookId); // 设置数据
