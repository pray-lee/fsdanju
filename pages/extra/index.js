...
Page({
  ...
  onAddExtra(e) {
    ...
    if (!!e) {
      index = e.currentTarget.dataset.index; // 校验
    ...
    if (this.data.subjectExtraConf) {
      ...
      if (!index && index !== 0) {
        tempData.extraList.push({
          conf: obj.array
        });
        tempData.extraMessage.push(obj.extraMessage);
      } else {
        tempData.extraList.splice(index + 1, 0, {
          conf: obj.array
        });
  ...
  onCopyExtra(e) {
    ...
    baoxiaoDetail.extraList.splice(index + 1, 0, {
      conf: obj.array
    });
  ...
  onExtraDateFocus(e) {
    ...
    var extraIdx = e.currentTarget.dataset.extraindex; // wx.datePicker({
    //     format: 'yyyy-MM-dd',
  ...
  onExtraBlur(e) {
    ...
    console.log(name);
    var tempData = clone(this.data.baoxiaoDetail); // 验证数字

    const field = tempData.extraList[extraIdx].conf[idx].collectStatus;
    const numberReg = /^\d+(\.\d+)?$/;

    if (field == '1') {
      app.globalData.caculateIndex = idx; // 验证数字
  ...
  onExtraInput(e) {
    ...
    var extraIdx = e.currentTarget.dataset.extraindex;
    var tempData = clone(this.data.baoxiaoDetail);
    tempData.extraMessage[extraIdx][idx] = e.detail.value; // 算附加信息金额
