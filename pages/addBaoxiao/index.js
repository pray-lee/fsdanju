...
Page({
  ...
  onShow() {
    ...
    this.getBorrowIdFromStorage(); // 从缓存里获取baoxiaoDetail
        ...
        if (res.data.length) {
          ...
          if (billDetailList && billDetailList.length) {
            var baoxiaoList = billDetailList.map(item => {
              ...
              obj.subjectName = item.subject.fullSubjectName, obj.subjectExtraId = subjectExtraId;
              ...
              obj.formatApplicationAmount = formatNumber(Number(item.applicationAmount).toFixed(2)); // 附加信息
              ...
              if (!!item.extraMessage) {
                ...
                if (obj.extraMessage && obj.extraMessage.length > 0) {
                  obj.extraMessage.forEach(item => {
                    extraList.push({
                      conf: this.generateExtraList(obj.subjectExtraConf).array
                    });
                  });
  ...
  setTotalAmount() {
    // 申请报销金额
    var applicationAmount = this.setApplicationAmount(this.data.baoxiaoList) || 0; // 核销金额
