...
Page({
  ...
  formSubmit(e) {
    ...
    console.log(this.data.submitData); // 删除辅助核算的信息，然后通过formatSubmitData重新赋值
    ...
    console.log(this.data.fukuanList, '------'); // 处理一下提交格式

    this.formatSubmitData(this.data.fukuanList, 'billDetailList'); // 提交的时候删除借款科目
  ...
  getBorrowIdFromStorage() {
    // 从缓存里获取借款人id
    const borrowId = tt.getStorageSync('borrowId');
    ...
    if (!!borrowId) {
      ...
      this.setApplicationAmount(this.data.fukuanList);
      this.setTotalAmount();
      this.getIncomeBankList(this.data.submitData.applicantType, borrowId); // 清理借款人缓存
  ...
  getImportFukuanListFromStorage() {
    const fukuanList = tt.getStorageSync('importCommonList');
  ...
  onShow() {
    ...
    // 从缓存获取修改的付款详情
    this.getFukuanDetailFromStorage(); // 从缓存获取付款列表

    this.getImportFukuanListFromStorage(); // 从缓存里获取借款列表

    this.getSelectedBorrowListFromStorage(); // 从缓存里获取借款人id
  ...
  getIncomeBankList(applicantType, applicantId, incomeBankName) {
    ...
    request({
      ...
      success: res => {
        ...
        if (arr.length) {} // edit的时候，设置incomeBankIndex
  ...
  borrowInput(e) {
    ...
    var tempData = clone(this.data.importList); // 本次核销金额

    tempData[index].applicationAmount = value;
    tempData[index].formatApplicationAmount = formatNumber(Number(value).toFixed(2));
    const newImportList = this.caculateImportList(tempData, value, index); // 验证输入
  ...
  setTotalAmount() {
    // 申请报销金额
    var applicationAmount = this.setApplicationAmount(this.data.fukuanList) || 0; // 核销金额
  ...
  getProcessInstance(billId, accountbookId) {
    ...
    request({
      hideLoading: this.hideLoading,
      url: app.globalData.url + 'feishuController.do?getProcessinstanceJson&billType=3&billId=' + billId + '&accountbookId=' + accountbookId,
      method: 'GET',
      success: res => {
        console.log(res, '审批流');
