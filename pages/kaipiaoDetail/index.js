...
Page({
  ...
  onLoad() {
    ...
    const isEdit = tt.getStorageSync('edit');
    const initKaipiaoDetail = tt.getStorageSync('initKaipiaoDetail');
    const kaipiaoDetail = tt.getStorageSync('kaipiaoDetail');
  ...
  getAuxptyIdFromStorage() {
    // 从缓存里获取auxpty
    const auxpty = tt.getStorageSync('auxpty');
  ...
  getSubjectIdFromStorage() {
    // 从缓存里获取科目id
    const subject = tt.getStorageSync('subject');
  ...
  onShow() {
    ...
    let kaipiaoDetail = tt.getStorageSync('kaipiaoDetail');
  ...
  setSelectedAuxpty(auxpty) {
    this.setData({
      kaipiaoDetail: { ...this.data.kaipiaoDetail,
        selectedAuxpty: { ...this.data.kaipiaoDetail.selectedAuxpty,
          [auxpty.auxptyId]: {
            id: auxpty.id,
            name: auxpty.name,
  ...
  addDetail() {
    const subjectList = tt.getStorageSync('subjectList');
    ...
    if (validSuccess) {
      ...
      this.setData({
        kaipiaoDetail: tt.getStorageSync('initKaipiaoDetail'),
        remarkIndex: 0
      });
    }
  },
